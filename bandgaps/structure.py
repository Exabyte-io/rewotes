...
converter = CrystalGraph(
    bond_converter=GaussianDistance(np.linspace(0, R_CUT, R_BASIS), R_SMEAR)
)

model = MEGNetModel(R_BASIS, graph_converter=converter)
...
df_test["pred_band_gap"] = model.predict_structures(df_test["final_structure"].tolist())
