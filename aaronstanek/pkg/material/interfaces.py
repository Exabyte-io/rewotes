...
class MaterialInterface(ABC):
    ...
    def load_from_file(filename: str) -> MaterialInterface:
        '''Load this object's binary format from a file, and reconstruct the object in memory.'''
        raise NotImplementedError()
...
class MaterialArchiveInterface(ABC):
    ...
    def load_from_file(filename) -> MaterialArchiveInterface:
        '''Load this collection's binary format from a file, reconstructing the collection in memory.'''
        raise NotImplementedError()

    def to_numpy_double_array(self) -> numpy.ndarray:
        '''Convert this collection to a 2D numpy array, where each row represents the properties of a material in the collection.'''
        raise NotImplementedError()

    def save_as_numpy(self, filename: str) -> None:
        '''Convert this collection to a 2D numpy array and save the result to a file.'''
        raise NotImplementedError()
