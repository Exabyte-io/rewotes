...
def get_parser() -> ArgumentParser:
    parser = ArgumentParser(description="Run basis set selection")
    parser.add_argument(
        "--xyz_path",
        type=str,
        required=True,
    ...
    parser.add_argument(
        "--property",
        type=str,
        choices=set(model.CalculatedReferenceProperty.__members__.keys(),).union(
    ...
    parser.add_argument(
        ...
        type=str,
        default="B3LYP",
        help="target level of theory for subsequent calculation (e.g. 'B3LYP')",
    )
    parser.add_argument(
        "--reference_theory",
        type=str,
        default="CCSD(T)",
        help="reference level of theory to use as benchmark (e.g. 'CCSD(T)')",
    )
    parser.add_argument(
        "--reference_basis",
        type=str,
        default="cc-pVDZ",
        help="basis set to use for selecting reference value (e.g. 'cc-pVDZ')",
    )
    parser.add_argument(
        "--tolerance",
        type=float,
...
def process_args(args: Namespace) -> model.Execution:
    ...
    with open(args.xyz_path, "r") as f:
        xyz_data = [ln.strip().split() for ln in f.readlines()[2:]]
    return model.Execution(
        xyz_data=xyz_data,
        property=property,
        regime=args.regime,
        target_theory=args.target_theory,
        reference_theory=args.reference_theory,
