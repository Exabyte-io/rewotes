...
def validate_property(regime: str, property: str):
    if regime == ReferenceRegime.experimental.value:
        raise NotImplementedError("Introduce support for CCCBDB experimental data")
    typ = (
        ExperimentalReferenceProperty
        if (regime == ReferenceRegime.experimental.value)
    ...
    try:
        ...
        raise Exception(f"property {property} not supported in regime {regime}")
...
class Execution(BaseModel):
    ...
    property: Union[
        ExperimentalReferenceProperty,
        CalculatedReferenceProperty,
    ]
    target_theory: str
    reference_theory: str
    reference_basis: str
    regime: ReferenceRegime
    value: Optional[float] = None
    tolerance: Optional[float] = 1.0

    @validator("reference_basis")
    def basis_to_lower(cls, v):
        return v.lower()

    def acceptable_range(self) -> Tuple[float, float]:
        if self.value is None:
            return None
        lower = (1 - self.tolerance / 100) * self.value
        upper = (1 + self.tolerance / 100) * self.value
        return lower, upper
    ...
    def xyz_data_to_dict(self) -> Dict[str, List[Dict[str, Any]]]:
        ...
        for i, (sym, *val) in enumerate(self.xyz_data):
            ...
            coordinates.append({"id": i, "value": [v * ang2au for v in val]})
        return {
            "elements": elements,
